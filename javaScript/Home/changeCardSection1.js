  ...
  if (!window.supabase) {
    console.error('Supabase is not loaded. Add the Supabase CDN script before your JS file.');
    return;
  }
  ...
  function getCoverUrlFromId(id) {
    ...
    // final path = covers/manga-20th-century-boys-1999.jpg
  ...
  async function updateHeroCard(item) {
    ...
    // Start loading the new image while the old card fades out
    const imageReady = preloadImage(coverUrl);

    // Start the visual change effect
    if (cardEffect) {
      cardEffect.classList.add('is-changing');
      cardEffect.classList.remove('is-revealing');
    }

    // Let the old content fade/blur out
    await wait(350);

    // Avoid waiting forever if the image is slow
    await Promise.race([
      imageReady,
      wait(1200)
    ]);

    // If another card change started, stop this one
  ...
  async function loadFeaturedManga() {
    ...
    // Only rotate if there is more than one featured manga
